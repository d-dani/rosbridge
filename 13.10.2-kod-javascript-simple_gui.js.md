...
Напомним, что функция refreshPublishers\(\) запускает таймер, который мы установили ранее, с частотой раз в секунду. Сначала мы запускаем функцию pubCmdVel\(\) \(описанную ниже\), чтобы опубликовать текущее сообщение Twist для управления базой робота. Затем мы проверяем, является ли тема chatterTopic в настоящее время объявленной, и если да, мы публикуем текущее сообщение chatter. 
...
Функция stopRobot\(\) довольно понятна. Хотя явный вызов pubCmdVe \(\) не является строго необходимым из-за нашего повторяющегося pubLoop\(\), он также не причиняет вреда, и лучше всего перестраховаться, для того, чтобы всегда иметь возможность остановить робота. 
